...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <p className="py-2 flex w-full  bg-[#FF0000] text-white md:text-sm text-xs font-light items-center justify-center">
        <span className="w-[90%] flex items-center justify-center">
          {Alert?.map((item: any, i: any) => (
            <div key={i} className="">
              {item?.show && <p>{item?.title}</p>}
            </div>
          ))}
        </span>
      </p>
                ...
                {isValidating ? (
                  ...
                  data?.map((item: any, i: number) => (
                    <div
                      ...
                      key={i}
                      className={`flex items-center gap-4 bg-gray-100  duration-200 rounded-md p-3 ${
                        router?.query?.name === item?.name
                          ? `bg-primary/40`
                          : `hover:bg-primary/40`
                      }`}
                    >
